...
export function listCronTools() {
    return cronToolInfo;
}
// Handle calls for cron-related tools
export async function handleCronToolCall(name, args, pb) {
    switch (name) {
        case 'list_cron_jobs':
            return listCronJobs(args, pb);
        case 'run_cron_job':
            return runCronJob(args, pb);
        default:
...
async function listCronJobs(args, pb) {
    const { fields } = args;
    try {
...
async function runCronJob(args, pb) {
    if (!args.jobId) {
