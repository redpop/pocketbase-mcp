...
export async function handleToolCall(params: CallToolRequest['params'], pb: PocketBase): Promise<ToolResult> {
    ...
    } else if (name === 'create_migration' || name === 'create_collection_migration' || name === 'add_field_migration' || name === 'list_migrations' || name === 'apply_migration' || name === 'revert_migration' || name === 'apply_all_migrations' || name === 'revert_to_migration' || name === 'set_migrations_directory') {
